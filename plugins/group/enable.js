...
let handler = async (m, { conn, usedPrefix, command, args, isOwner, isBotAdmin, isAdmin, isROwner }) => {
	...
	switch (type) {
		...
		case 'psgc':
			...
			break
		default:
			if (!/[01]/.test(command)) return m.reply(`*List option :*\n| presence | welcome | delete | antidelete | autolevelup | ephemeral | nsfw | simi | game | anticall | antisticker | antitoxic | antilink | antilinkkick | antiuncheck | antivirtex | antiviewonce | autoai | autoaipc | newstoram | public | self | restrict | autoread | adminonly | owneronly | pconly | gconly |
