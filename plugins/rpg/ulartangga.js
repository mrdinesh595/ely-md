...
class SnakeAndLadderGame {
	...
	async playTurn(m, player) {
		if (!this.players.length) {
			await m.reply(`🛑 *Tidak ada permainan aktif.* Gunakan "${usedPrefix}${command} start" untuk memulai permainan baru.`);
			return;
		}
...
const handler = async (m, {
	...
	switch (args[0]) {
		...
		case 'roll':
			if (!state) return m.reply(`🛑 *Permainan belum dimulai.* Ketik "${usedPrefix}${command} start" untuk memulai.`);
			if (game.isGameStarted()) {
				...
				await m.reply(`🛑 *Permainan belum dimulai.* Ketik "${usedPrefix}${command} start" untuk memulai.`);
			}
			break;
...
handler.menufun = ['ulartangga', 'ladders', 'snake']
handler.tagsfun = ['rpg']
handler.command = /^(ulartangga|ladders|snak(e)?)$/i;
