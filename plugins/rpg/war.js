...
let handler = async (m, { conn, usedPrefix, args, command }) => {
    ...
    async function cekAFK(x) {
        ...
        if (turn == turnNow && time == timeNow) {
            ...
            await conn.reply(m.chat, `*@${conn.war[m.chat][turn].user.split('@')[0]} sedang AFK (Denda -2500 HP)*\n\n.war player = statistik pemain\n.attack @tag = serang lawan`, null, { contextInfo: { mentionedJid: [conn.war[m.chat][turn].user] } })
            await sleep(3000)
            // cek kalau mati
            if (conn.war[m.chat][turn].hp <= 0) {
                await conn.reply(m.chat, `*@${conn.war[m.chat][turn].user.split('@')[0]} sudah mati karena HP (Health Point) habis.*`, null, { contextInfo: { mentionedJid: [conn.war[m.chat][turn].user] } })
                // cek tim nya
                let playerTotal = 0
                let playerKalah = 0
                if (turn < 5) {
                    ...
                    if (playerTotal > 0 && playerTotal == playerKalah) {
                        ...
                        await conn.reply(m.chat, `*TEAM B MENANG KARENA TEAM A GOBLOK SEMUA*\n\n*TEAM A :*\n` + teamA.map((v, i) => `${conn.war[m.chat][i].hp > 0 ? '❤️ ' : '☠️ '}@${v.split('@')[0]} (- ${Number(conn.war2[m.chat].money).toLocaleString()} Money )`).join`\n` + "\n\n*TEAM B :*\n" + teamB.map((v, i) => `${conn.war[m.chat][i + 5].hp > 0 ? '❤️ ' : '☠️ '}@${v.split('@')[0]} (+ ${Number(conn.war2[m.chat].money).toLocaleString()} Money )`).join`\n`, m, {
                            contextInfo: {
                                mentionedJid: teamAB
                            }
                        })
            ...
            await sleep(3000)
            await conn.reply(m.chat, `*Giliran @${conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0]} untuk menyerang (Waktu 90 detik)*\n\n.war player = statistik pemain\n.attack @tag = serang lawan`, null, { contextInfo: { mentionedJid: [conn.war[m.chat][conn.war2[m.chat].turn].user] } })
            cekAFK(conn.war2[m.chat].turn)
        }
    }

    // Check if player join in the war
    function isPlayerInGame(sender) {
        for (let i = 0; i < 10; i++) {
            if (conn.war[m.chat] && conn.war[m.chat][i].user == sender) {
                return true
            }
        }
        return false
    ...
    if (!isPlayerInGame(m.sender)) return m.reply(`*Kamu bukan peserta war.*`)

    for (let i = 0; i < 10; i++) {
        if (m.sender == conn.war[m.chat][i].user) {
            if (i != conn.war2[m.chat].turn) {
                await conn.reply(m.chat, `*Sekarang adalah giliran @${conn.war[m.chat][conn.war2[m.chat].turn].user.split('@')[0]} untuk menyerang.*`, m, { contextInfo: { mentionedJid: [conn.war[m.chat][conn.war2[m.chat].turn].user] } })
                cekAFK(conn.war2[m.chat].turn)
            }
        }
    }
            ...
            if (conn.war[m.chat][i].user == args[0] && conn.war[m.chat][i].hp > 0) {
                ...
                for (let i = 0; i < 10; i++) {
                    if (conn.war[m.chat][i].user == target) {
                        ...
                        await conn.reply(m.chat, `*@${attacker.split('@')[0]} menyerang @${target.split('@')[0]} sampai nyawanya berkurang ${pointAttacker * 500} (Sisa HP: ${conn.war[m.chat][i].hp})*\n\n*@${attacker.split('@')[0]} [${pointAttacker * 10}%] - [${pointTarget * 10}%] @${target.split('@')[0]}*\n*Level sangat mempengaruhi keberhasilan.*`, m, { contextInfo: { mentionedJid: [attacker, target] } })
                        await sleep(2000)
                        if (conn.war[m.chat][i].hp <= 0) await conn.reply(m.chat, `*@${target.split(`@`)[0]} sudah mati dalam pertarungan.*`, m, { contextInfo: { mentionedJid: [target] } })
                        success = true
                    }
                }
            }
