...
let handler = async (m, { conn, usedPrefix, args, command }) => {
  ...
  *${usedPrefix + command} start* = start game`)
  ...
  if (args[0] == "money") {
    ...
    if (m.sender == conn.war[m.chat][0].user) {
      if (args[1] != "undefined" && !isNaN(args[1])) {
        ...
        if (args[1] < 100000) return m.reply('*Minimal 100.000 Money*')
        conn.war2[m.chat].money = args[1]
        return m.reply("*Berhasil menetapkan modal perang sebesar " + Number(args[1]).toLocaleString() + " Money*")
      } else {
  ...
  if (args[0] == "join") {
    ...
    if (db.data.users[m.sender].money < 10000) return m.reply("*Minimal memiliki 10.000 Money untuk bermain game ini.*")
    // FIRST PLAYER
    if (!(m.chat in conn.war)) {
      conn.war2[m.chat] = { "war": false, "turn": 0, "time": 0, "money": 0 }
      conn.war[m.chat] = []
      let exp = db.data.users[m.sender].exp
      conn.war[m.chat][0] = { "user": m.sender, "hp": 5000, "lvl": db.data.users[m.sender].level, "turn": false }
      for (let i = 1; i < 10; i++) {
      ...
      if (args[1]) {
        if (args[1].toLowerCase() == "a") {
          if (conn.war2[m.chat].money == 0) return conn.reply(m.chat, `*Tolong @${conn.war[m.chat][0].user.split('@')[0]} tetapkan modal awal perang (minimal 100.000 Money)*\n\n.war money 100000`, m, { contextInfo: { mentionedJid: [conn.war[m.chat][0].user] } })
          if (db.data.users[m.sender].money < conn.war2[m.chat].money) return m.reply(`*Minimal memiliki ${conn.war2[m.chat].money.toLocaleString()} Money untuk bermain game ini.*`)
          for (let i = 1; i < 5; i++) {
            if (conn.war[m.chat][i].user == "") {
              let exp = db.data.users[m.sender].exp
              conn.war[m.chat][i] = { "user": m.sender, "hp": 5000, "lvl": db.data.users[m.sender].level, "turn": false }
              let total = 0
              for (let i = 0; i < 10; i++) {
                if (conn.war[m.chat][i].user == "") {
                  total += 1
...
if (args[0] == "left") {
  ...
  // IF GAME STARTED
  if (conn.war2[m.chat].war) {
    return m.reply(`*Perang sudah dimulai, anda tidak bisa keluar*`)
  }
  ...
  if (left) {
    ...
    let allLeft = conn.war[m.chat].every(player => player.user === "")

    if (allLeft) {
      // Delete the room if no players are left
      delete conn.war[m.chat]
      delete conn.war2[m.chat]
      return m.reply(`*Semua pemain telah keluar, room telah dihapus*`)
    } else {
  ...
  // CEK PLAYER
  if (args[0] == "player") {
    if (!(m.chat in conn.war) || !(m.chat in conn.war2)) return m.reply(`*Tidak ada pemain yang join room War Zone*`)
    var teamA = []
    var teamB = []
    var teamAB = []
    for (let i = 0; i < conn.war[m.chat].length; i++) {
