...
let handler = async (m, { conn, usedPrefix, command }) => {
	try {
		...
		} else await conn.sendMsg(m.chat, { image: {url: anu.url}, caption: `_Random pic: ${command}_\n${anu.anime_name ?? anu.source_url}` }, { quoted: m })   // await conn.sendButton(m.chat, `_Random pic: ${command}_\n${anu.anime_name ?? anu.source_url}`, pauthor, anu.url, [[command, usedPrefix+command]], m) 
	} catch (e) {
		console.log(e)
