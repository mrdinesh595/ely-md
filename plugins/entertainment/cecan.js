...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	try {
		...
		await conn.sendMsg(m.chat, { image: { url: anu.getRandom() }, caption: `Cecan > ${text}` }, { quoted: m })    // conn.sendButton(m.chat, `Cecan > ${text}`, pauthor, anu.getRandom(), [[text, usedPrefix+text]], m)
	} catch (e) {
		console.log(e)
